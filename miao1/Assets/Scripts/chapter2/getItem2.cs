...
public class getItem2 : MonoBehaviour
{
    ...
    {
        /*
        if(GameManager2.instance.items.Contains(obj) && !finish)
        {
        ...
        */
        if(GameManager2.instance.items.Contains(_items[3]))
        {
            finish[0] = true;
        }
        for(int i=0; i<_items.Count; i++)
        {
            if(GameManager2.instance.items.Contains(_items[i]) && !finish[i])
            {
                showTip(i);
            }
        }
